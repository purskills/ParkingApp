...
class MyParking extends Component {
  ...
  parkingDetails = () => {
    this.props.navigation.navigate('CurrentParkingDetailsScreen');
  }
  
  renderMyParkingItem = ({ item }) => {
    let { startTime, endTime } = item;
    const hours = moment.duration(moment(endTime, 'HH:mm a')
                        .diff(moment(startTime, 'HH:mm a')))
                        .asHours().toString().substr(0, 3);
    return (
      <View style={styles.parkingItem}>
        ...
        <View style={styles.parkingRight}>
          ...
          />
          <Text
            text={`${item.startTime} - ${item.endTime} (${hours} hours)`}
            size={12}
            type="light"
            marginTop={10}
          /> 
          ...
          <View style={{ marginTop: 10, alignItems: 'flex-end' }}>
            <TouchableOpacity style={{ flexDirection: 'row' }} onPress={() => this.parkingDetails()}>
              <Text
                text="More details"
